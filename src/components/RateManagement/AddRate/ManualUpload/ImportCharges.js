...
const ImportCharges = ({
    ...
    const handleChange = (id, name, value) => {
        const chargesCopy = [...importCharges];
        ...
        setImportCharges(chargesCopy);
    };

    const addCharge = () => {
        setImportCharges((prevState) => {
            return [
                ...prevState,
                {
                    label: "",
    ...
    return (
        <div className="w-full">
            {!editMode && (
                <p className="text-[22px] font-rocGroteskBold mb-6">Import documentation charges</p>
            )}
            <div>
                {importCharges?.map((charge, idx) => {
                    return (
