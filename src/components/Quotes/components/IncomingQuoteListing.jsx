...
const IncomingQuoteListing = () => {
    const bottom = useRef(null);
    const entry = useIntersectionObserver(bottom, {});
    const isVisible = !!entry?.isIntersecting;
    const { data, isLoading, hasNextPage, isFetchingNextPage, fetchNextPage } = useInfiniteQuery({
        queryKey: [`${quoteConstants.FETCHING_INCOMING_QUOTE_REQUEST}`],
        queryFn: ({ pageParam = 1 }) => quoteService.fetchIncomingQuoteRequests(pageParam),
...
export default IncomingQuoteListing;
